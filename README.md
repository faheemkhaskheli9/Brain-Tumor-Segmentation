...
![Sample scan, ground-truth mask, and predicted mask side by side](docs/sample_output.png)

Left to right: the synthetic sample scan, its ground-truth mask, and the
model's predicted mask, generated by running `scripts/segment.py` on the
sample image and combining the result via
`scripts/generate_result_preview.py`.

The model here is built with `encoder_weights=None` and this repo has no
trained-weights checkpoint or real dataset committed to it (see the
disclosure above and `docs/evaluation.md`), so this image demonstrates the
preprocess -> predict -> write pipeline running end to end, not
segmentation accuracy -- a real Dice/IoU number would require training
against the actual Kaggle dataset outside this repo.
